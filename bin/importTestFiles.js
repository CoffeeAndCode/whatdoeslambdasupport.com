...
function getTestFile(fileName) {
  request(testFileBaseURL + fileName).pipe(fs.createWriteStream(path.join(__dirname, '../functions/app/tests', fileName)));
}
...
fs.writeFileSync(path.join(__dirname, '../functions/app/tests/timestamp.js'), 'module.exports = "' + new Date() + '";');
